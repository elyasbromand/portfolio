...
> **Status: implemented, awaiting the user's review before commit.**
>
> User confirmed (asked before touching `SectionHeading`, since it was ambiguous after the Phase 2 reset): the scramble-text effect stays for **one-time, scroll-into-view reveals** — section index numbers, metric values, etc. It's only the *on-hover* use (nav links) that was rejected; that's already gone from Nav and isn't reintroduced here.

- [x] **`SectionHeading.tsx`** (stays a server component, adds attributes): the index `01` gets `data-reveal="scramble"`, the `h2` gets `data-reveal="lines"`, and the rule gets `data-reveal="rule"` with `data-reveal-delay="0.15"`. This one change animates every heading on both routes (verified: all 4 homepage sections + all 3 case-study sections reveal correctly, both when scrolled to individually and when the page jumps straight to the bottom in one go).
- [x] **`lib/useCountUp.ts`** rewritten with GSAP. Signature: `useCountUp(target, { ref, duration = 1.4, decimals = 0 })` (seconds, not the old ms). Tweens a proxy `{v:0}` with `ease: "power2.out"`, `snap: { v: 1 }` when `decimals` is 0 (GSAP's snap special property takes a `{prop: increment}` object, not a bare string as originally sketched), triggered by `ScrollTrigger({ trigger: ref, start: "top 85%", once: true })`. Reduced motion checks `window.matchMedia("(prefers-reduced-motion: reduce)")` directly and jumps straight to the final value. `tabular-nums` added via `fontVariantNumeric` on both consumers so the digit width doesn't jitter mid-count.
- [x] **`Metrics.tsx`** drops its duplicated rAF loop; its 3 counters share one `gridRef` (all three cards always enter the viewport together as one row, so one trigger is enough — no need for a ref per card). **`ProjectStats.tsx`** gives each `StatCard` its own ref instead, since case-study stat counts vary and a shared trigger wouldn't generalize as well there.
- [x] **Verified with real (wheel-driven) scrolling, not a synthetic jump:** a number stays at 0 while below the fold, is demonstrably mid-count partway through scrolling it into view (e.g. `29` before settling on the final `30`), and reaches the exact target shortly after. (A first attempt at this check used `element.scrollIntoView()` and wrongly looked broken — that API bypasses ScrollSmoother's own scroll handling entirely and isn't something a real user ever triggers; switching the test to real wheel events showed the hook working correctly all along.) Reduced motion shows every counter at its final value from the first frame.

**Bug found and fixed (pre-existing, from Phase 1, first triggered by this phase):** `MotionRoot`'s `data-reveal="lines"` handler lived inside the same `matchMedia` branch as the width/hover-dependent reveals, which fully reverts and recreates on every 860px crossing. But `SplitText`'s `autoSplit: true` already re-splits internally on resize — recreating the *whole SplitText instance* again on top of that raced with its own internal resplit and crashed `ScrollTrigger.refresh()` (a stale trigger's `.end` went `undefined`, escalating into a `RangeError: Maximum call stack size exceeded` on the next resize, which unmounted the entire React tree). Reproduced by: opening the phone menu, scrolling, navigating via a link, closing the menu, then resizing past 860px. **Fix:** `data-reveal="lines"` now lives in its own `matchMedia` branch keyed only on `reduceMotion`/`motionOK` — created once, left alone, with `autoSplit` owning every future resplit natively. The mobile/desktop-dependent stagger and duration are read fresh inside `onSplit` via `window.matchMedia(mq.isMobile)` rather than a frozen closure, so they still track the current breakpoint. Stress-tested with 9 rapid width changes crossing 860px back and forth: 0 errors, all 4 headings still correctly split and visible afterward.

## Phase 4 — Hero (`components/Hero.tsx`)

> **Status: implemented, awaiting the user's review before commit.**

Load timeline (not scroll-triggered). It runs after hydration and `document.fonts.ready` and finishes well under 1.5s (the readable content — badge/h1/lead/typed line/CTAs — settles by ~1.3s; only the small portrait name-tag flourish runs slightly past that), since the `h1` is the likely LCP element. `Hero.tsx` stays a server component; it wraps its markup in a client `HeroMotion` component (`components/motion/HeroMotion.tsx`), which owns the actual `<header>` element (not a `display:contents` wrapper — the scroll-out parallax below needs a real bounding box for its ScrollTrigger). This is a hand-built master timeline, not the generic `MotionRoot` reveal engine — Hero needs precise cross-element sequencing and effects (the typed line, the portrait's clip-path wipe) that engine doesn't cover. Elements are tagged `data-hero="..."` and hidden via `[data-js] [data-hero]{visibility:hidden}` in `globals.css` (the same mechanism as `[data-reveal]`, under its own attribute).

- [x] 0.00 — badge: scale .9 → 1 and autoAlpha. The green dot gets an infinite "ping" ring — a real `::after` in CSS (`Hero.module.css`), not GSAP: it's a simple ambient loop with no coordination needs, consistent with the project's existing plain-CSS `blink`/`sparkDraw` keyframes. Disabled under reduced motion.
- [x] 0.10 — `h1`: SplitText `lines` + `mask: "lines"`, yPercent 105 → 0, stagger .09, `expo.out` 1.0s, `autoSplit` for later resizes. *Built:* the reveal animation is explicitly nested into the master timeline via `tl.add(anim, 0.1)` (returning it from `onSplit` alone does **not** place it in the timeline at the right offset — it just runs on its own independent schedule). Only the *first* `onSplit` firing gets this reveal; later ones (a real resize, well after the intro has played) just snap new line elements to their resting position with no animation, avoiding a second replay.
- [x] 0.45 — the lead paragraph fades up (y 16).
- [x] 0.60 — the `// Information Systems…` mono line types in (TextPlugin, ~0.7s) with a blinking caret (CSS `blink`, matching the site's other status-dot blinks) that fades out afterwards. *Built:* the caret is a **sibling** span, not a child of the typed line — clearing the typed line's `textContent` before typing (a plain DOM write, not a TextPlugin `.set()`, for certainty) would otherwise delete a nested caret along with everything else.
- [x] 0.70 — CTAs stagger up. "View selected work" gets `data-magnetic` (picked up automatically by `MotionRoot`'s existing pointer-effects scan — no bespoke code needed). Its `→` nudge on hover is a plain CSS transition (`.primaryCta:hover .arrow`), not GSAP — nothing else animates that span's transform, so CSS is simpler and there's nothing to conflict with (rule 5).
- [x] 0.20 — portrait: `frame`'s `clipPath: inset(100% 0% 0% 0%) → inset(0% 0% 0% 0%)` (1.1s, `power4.inOut`), `image` scales 1.25 → 1 at the same time, `border` fades in. Name tag (`tag`) slides in from x -12 at 0.9.
- [x] **Scroll-out (desktop, scrub 0.5):** `image` gets `yPercent -8`, the text column gets `y -40, autoAlpha .3`, both on one shared `ScrollTrigger` (`start: "top top", end: "bottom top"`) tied to `HeroMotion`'s own `<header>`.
- [x] **Hover (`canHover`):** 3D tilt on `frame` (`quickTo rotationX/Y`, clamped ±6°, `transformPerspective: 800` set directly on the tilted element itself — a documented GSAP convenience that avoids needing a separate perspective-establishing wrapper) plus the existing `data-spotlight` glare (same shared wiring as the CTA's magnetic pull). Resets to 0/0 on pointer leave.
- [x] **Mobile:** the whole load sequence runs at `MOBILE_TIME_SCALE` (0.8×) — computed once via `window.matchMedia(mq.isMobile).matches` at setup (not a reactive `matchMedia().add()`, since this is a one-shot load animation that must never replay on a later resize). No parallax, no tilt (both are separately matchMedia-gated to desktop/fine-pointer only, and — unlike the load timeline — *are* allowed to react to matchMedia changes normally, since they're ongoing behaviours, not one-shot).

**Four real bugs found and fixed.** The first round of automated testing (below) passed cleanly but checked computed `visibility`/`opacity` on the *outer* elements only — not whether the actual rendered pixels matched, which is exactly how bugs 3 and 4 slipped through initially and were only caught after the user reported the headline text and portrait image weren't visible. Screenshots and, for bug 4, dumping the actual per-line transform values were what actually caught them; that's now part of how this phase is verified (see below).

1. **Every text-column element stayed invisible forever** (and consequently un-hoverable/un-clickable — the magnetic CTA test read 0px offset because the browser excludes `visibility: hidden` elements from hit-testing entirely). Cause: the wrapper `<div>` needed so the scroll-parallax effect could find the text column was tagged `data-hero="textcol"` — the *same* attribute the CSS reveal-gate matches. Since that wrapper was never itself given a reveal tween, it stayed `visibility: hidden` forever from CSS; GSAP's `autoAlpha` on its *children* sets `visibility: inherit` (not literally `"visible"`), so they inherited the parent's hidden state right back, regardless of their own opacity reaching 1. **Fix:** renamed it to a separate, non-colliding attribute (`data-parallax="text"`) that the CSS gate doesn't match at all, since it isn't itself a reveal target.
2. Several `.set()`/`.fromTo()` calls omitted an explicit timeline position, which made GSAP default them to "right after whatever was added last" instead of the intended absolute offset — harmless for the very first call in the file, but wrong for ones following the h1 SplitText insert. Fixed by giving every single timeline call in the sequence an explicit `at(seconds)` position, with a comment noting why it matters here specifically.
3. **The portrait image itself was permanently invisible** (reported by the user — bug 1's fix made everything else in the hero visible, but this one is a separate element with the same root cause pattern). Its own tween only animated `scale`, never `autoAlpha`, so the CSS gate's `visibility: hidden` — a *direct* match on the `<img>`, not something inherited from a parent — was never lifted. **Fix:** added `image` alongside `frame` in the `.set([frame, image], { autoAlpha: 1 }, at(0.2))` call.
4. **The headline text itself was permanently invisible**, also reported by the user. Two compounding causes, found by dumping the actual per-line computed transforms (all four lines stuck at `translateY` matching exactly their `yPercent: 105` *start* value, never having moved) and by temporary console logging:
   - React Strict Mode (dev only) mounts, cleans up, and remounts every effect once, synchronously, specifically to surface bugs like this one. The whole load timeline was built inside a `document.fonts.ready.then(...)` callback — an async continuation that `useGSAP`'s automatic revert-on-cleanup does **not** track (it only tracks `gsap.*` calls made synchronously inside the effect body). So the phantom first mount's callback still fired later and ran `SplitText.create()` a second time on the same `h1`, colliding with the real mount's split. Confirmed via logging: `onSplit` fired twice, each reporting a fresh `firstSplit=true`. **Fix:** a `cancelled` flag, set in the effect's cleanup and checked at the top of the async callback — the standard pattern for guarding async work started inside a React effect.
   - Independently, even a single correctly-created split never animated at all. The reveal tween was built *inside* `onSplit` and inserted into the already-running master timeline via `tl.add(anim, at(0.1))` — but `onSplit`'s *initial* firing apparently lands slightly later than the surrounding synchronous code (likely a frame or two, for layout measurement), by which point `tl`'s playhead had already advanced past the 0.1s insertion point. GSAP does not rewind a running timeline to render something inserted behind its current position, so the lines sat forever at their `.from()` start value. **Fix:** stopped trying to insert into a live timeline from inside the callback entirely. `SplitText.create()`'s *initial* split is synchronous, so `split.lines` is used directly in a plain `tl.from(split.lines, {...}, at(0.1))` — exactly like every other element in the sequence, no special-casing needed. `onSplit` is now used only for *later*, genuine re-splits (an actual resize, well after the intro already played), where it just snaps the new lines to their resting position with no animation.

**Verified:** timeline sampled at 12 points from t=0 to t=1700ms — badge/h1/lead/typed-text/caret/ctas/frame-clip/image-scale/tag all reveal in the right order and are genuinely `visibility: visible` throughout; the typed line is demonstrably mid-type at several samples before landing on the exact final string, and the caret fades out shortly after. A full-page screenshot at rest confirms the headline and portrait are both genuinely visible (not just passing a computed-style check) — this is now part of how this phase is checked, not just DOM-property assertions. The h1 lines' individual `translateY` was sampled across several points and shown converging to 0 with the intended per-line stagger (later lines trailing slightly behind earlier ones), confirming a real animation rather than an instant snap. Hover tilt applies a real 3D rotation (`is2D: false` on the computed transform matrix) and relaxes back toward identity on leave. Spotlight and magnetic both engage correctly (8px offset, matching the clamp). Scroll-out parallax matches the expected progress within the tolerance `scrub: 0.5`'s catch-up lag accounts for. Mobile completes the full sequence, faster. Reduced motion shows the final state immediately, with no typing, no tilt, no parallax, and the ping ring disabled. Full Phase 1–3 regression (anchor landings, nav hide/show, cursor, section-heading reveals, count-up timing, the SplitText/resize stress test) still passes with no errors.
